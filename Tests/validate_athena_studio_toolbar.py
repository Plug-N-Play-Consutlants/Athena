"""Validate Athena Studio toolbar refinement."""
...
REQUIRED_METHODS = {"_toolbar_button", "_button_group", "reload_patched_build", "refresh_studio_ui"}
REQUIRED_MARKERS = [
...
REQUIRED_ACTIONS = ["▶ Launch", "🔄 Reload", "🛑 Stop", "🌐 Open Scout", "🔃 Refresh", "⟲ Restart Studio"]
...
def main() -> int:
    print("Athena Studio Toolbar Validation")
    print("=" * 60)
    failures: list[str] = []
    if not STUDIO.exists():
        ...
        if missing_methods:
            failures.append("Missing toolbar methods: " + ", ".join(missing_methods))
        else:
            print(f"[PASS] required toolbar methods present: {len(REQUIRED_METHODS)}")
        missing_markers = [m for m in REQUIRED_MARKERS if m not in text]
        if missing_markers:
            failures.append("Missing toolbar markers: " + ", ".join(missing_markers))
        else:
            print(f"[PASS] required toolbar markers present: {len(REQUIRED_MARKERS)}")
        missing_actions = [a for a in REQUIRED_ACTIONS if a not in text]
        if missing_actions:
            failures.append("Missing toolbar action labels: " + ", ".join(missing_actions))
        else:
            print(f"[PASS] required toolbar action labels present: {len(REQUIRED_ACTIONS)}")
