...
def assess_capabilities(provider: str = "Fantrax") -> Dict[str, Any]:
    ...
    try:
        from Knowledge.Events.live_intelligence import live_intelligence_diagnostics
        live_diag = live_intelligence_diagnostics()
    except Exception as exc:  # noqa: BLE001 - capability dashboard must not fail on optional live layer
        live_diag = {"status": "failed", "feed_count": 0, "selected_count": 0, "error": f"{type(exc).__name__}: {exc}"}
    live_feed_count = int(live_diag.get("feed_count") or 0) if isinstance(live_diag, dict) else 0
    live_selected_count = int(live_diag.get("selected_count") or 0) if isinstance(live_diag, dict) else 0
    live_status = AVAILABLE if live_feed_count > 0 else MISSING
    live_reason = (
        f"RSS feeds configured: {live_feed_count}; validation/sample events available: {live_selected_count}."
        if live_feed_count > 0
        else "No RSS/live event feeds are configured."
    )
    capabilities.append(_capability(
        "live_event_sources", "Live event sources", live_status, layer="Events", required=False,
        reason=live_reason,
        impact="Required for current-news and recent-event Scout answers.",
        evidence=live_diag if isinstance(live_diag, dict) else {},
    ))
