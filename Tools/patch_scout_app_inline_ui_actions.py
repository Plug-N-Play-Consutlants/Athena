"""Inline-patch Scout/app.py with the UI action repair script.

Use this if /static/ui_action_repair.js is not served by Scout.
...
MARKER = "Scout UI Action Repair bound"
INLINE = r"""<script>
/*
...
def main():
    print("Inline patch Scout UI actions")
    print("=============================")
    text = APP.read_text(encoding="utf-8")

    if MARKER in text:
        print("[PASS] Inline UI action repair already present")
        return

    if "</body>" not in text:
        raise RuntimeError("Could not find </body> in Scout/app.py HTML template.")

    text = text.replace("</body>", INLINE + "\n</body>", 1)
    APP.write_text(text, encoding="utf-8")
    print("[PASS] Inline UI action repair inserted before </body>")
    print("Restart Scout after this patch.")
