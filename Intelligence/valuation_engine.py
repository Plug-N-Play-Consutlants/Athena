...
def _merge_contracts(players: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int]:
    ...
    for player in players:
        ...
        # Player profiles may contain stale or misinterpreted contract fields
        # from earlier builds. Strip them first, then add back only verified
        # contract knowledge from Output/player_contracts.json.
        ...
        has_contract_fact = bool(contract) and evidence_completeness > 0.0 and bool((contract or {}).get("has_verified_contract", True))

        if has_contract_fact:
            matched += 1
            merged["contract_expiry_year"] = contract.get("contract_expiry_year")
            merged["contract_years_remaining"] = contract.get("contract_years_remaining")
            merged["active_contract_season"] = contract.get("active_contract_season")
            merged["max_contract_years"] = contract.get("max_contract_years")
            ...
            merged["has_verified_contract"] = True
...
def _dimension_value(profile: dict[str, Any], dimension: str) -> tuple[float, list[str], bool]:
    ...
    if dimension == "contract":
        ...
        expiry_year = profile.get("contract_expiry_year")
        status = _safe_str(profile.get("contract_status"))
        if contract_completeness > 0.0 and score is not None:
            detail = "Contract score available from Fantrax expiry-year contract data"
            if expiry_year is not None:
                detail += f": expiry {expiry_year}"
            ...
            evidence.append(detail + ".")
            return score, evidence, True
        evidence.append("Verified contract data unavailable; using neutral placeholder with no contract penalty.")
        return _fallback_midpoint(), evidence, False
...
def evaluate_player(profile: dict[str, Any], analysis_profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "contract_expiry_year": profile.get("contract_expiry_year"),
...
def _flatten_for_csv(value: dict[str, Any]) -> dict[str, Any]:
    row = {
        ...
        "contract_expiry_year": value.get("contract_expiry_year"),
...
def build_player_values() -> list[dict[str, Any]]:
    ...
    log(f"Verified Contract Matches: {contract_matches}")
    ...
    log("  Values now consume NHL production, bio/age, and verified contract data when available.")
